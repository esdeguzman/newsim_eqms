...
class CparController extends Controller {
    ...
    public function show(Cpar $cpar) {
        $responsible = NA::user($cpar->person_responsible);
        $user = collect($responsible);
        $reporting = NA::user($cpar->person_reporting);
        $chief = NA::user($responsible->chief_id);
        ...
        return view('cpars.show', compact('cpar', 'sections', 'body', 'responsible', 'reporting', 'chief'));
    }
    ...
    public function graphData(Request $request) {
        return Cpar::select('id')->where('branch', $request->branch)
                                 ->where('');
    }
}
